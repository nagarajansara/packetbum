   	                                  ...
   	                                  <#assign user=retrieveMessage.vclid?string.number>
   	                                <li><strong><a href="#">${retrieveMessage.shortname} :</a></strong> <span><b>${retrieveMessage.creates?datetime?string["MM/dd hh:mm a"]}</b></span> ${retrieveMessage.message} <a rel="nofollow" onclick="flagReport(this)" data-item-id="${retrieveMessage.itemid}" data-message="${retrieveMessage.message}" data-id="${retrieveMessage.messageid}" style=" margin-left: 13px; cursor:pointer;" class="flag flagReport">&nbsp;</a></li>
                                               		...
                                               		<div class="messageBox">
                                               		 ...
                                               		 <form name="messageReplyForm" id="messageReplyForm">                                                           
                                                            <input type="hidden" name="messageid" value="${model['messageid']}"/>
                                                             <input type="hidden" value="${model['usershortname']}" id="usernameID"/>
...
"\n-Please either click OK to report a breach or cancel to exit.");
    	   ...
    	   if(con){
    		  ...
    		  // var messageid=	$(val).attr("data-message-id");
    		   ...
    		   $.post(link,{message: message, itemid:itemid,id:id});
